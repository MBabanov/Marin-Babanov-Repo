angular.module('ngSocketMock', []).service('ngSocketBackend', [function () {
    var connectQueue = [], pendingConnects = [], closeQueue = [], pendingCloses = [], sendQueue = [], pendingSends = [];
...
angular.module('ngSocket', []).service('ngSocketBackend', ['$window', function ($window) {
    this.createWebSocketBackend = function (url) {
    ...
    NGWebSocket.prototype.send = function (data) {
        ...
        var deferred = $q.defer(), socket = this, promise = cancelableify(deferred.promise);
    ...
    NGWebSocket.prototype.create_heardbreat = function () {
        this.heartbeat_interval = setInterval(this.send_heardbreat, 5000);
    }
    NGWebSocket.prototype.send_heardbreat = function () {
        try {
            this.missed_heartbeats += 1;
            if (this.missed_heartbeats > 5)
                throw new Error("Too many missed heartbeats.");
            this.socket.send('mike')
        } catch (e) {
